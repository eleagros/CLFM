...
class NNMatcher(object):
    ...
    def run(self, nkpts, descs):
        ...
        if torch.cuda.is_available():
          ...
          mutual_nearest = (nn_idx2[nn_idx1] == torch.arange(nn_idx1.shape[0]))
    ...
    def infer(self, kpt_list, desc_list):
        with torch.no_grad():
            if torch.cuda.is_available():
              ...
              nkpts = [torch.from_numpy(self.normalize_kpts(i[:,:2]).astype(np.float32)) for i in kpt_list]
              descs = [torch.from_numpy(desc.astype(np.float32)) for desc in desc_list]
