...
def align_imgJ(ij, path_tmp):
    ...
    macro = """
        ...
        run("bUnwarpJ", "load=""" + path_tmp_no_start + """/coordinates.txt source_image=img_x.tif target_image=img_x.tif registration=Accurate image_subsample_factor=0 initial_deformation=Coarse final_deformation=Fine divergence_weight=0.1 curl_weight=0.1 landmark_weight=1.5 image_weight=0 consistency_weight=10 stop_threshold=0.01 save_transformations save_direct_transformation=""" + path_tmp_no_start + """/global_img_direct_transf.txt save_inverse_transformation=""" + path_tmp_no_start + """/img_x_inverse_transf.txt");
        ...
        open(""" + path_tmp + """/img_y.tif");
        call("bunwarpj.bUnwarpJ_.loadLandmarks", """ + path_tmp + """/coordinates.txt");
        run("bUnwarpJ", "load=""" + path_tmp_no_start + """/coordinates.txt source_image=img_y.tif target_image=img_y.tif registration=Accurate image_subsample_factor=0 initial_deformation=Coarse final_deformation=Fine divergence_weight=0.1 curl_weight=0.1 landmark_weight=1.5 image_weight=0 consistency_weight=10 stop_threshold=0.01 save_transformations save_direct_transformation=""" + path_tmp_no_start + """/global_img_direct_transf.txt save_inverse_transformation=""" + path_tmp_no_start + """/img_y_inverse_transf.txt");
        selectWindow("Registered Source Image");
...
def main():
    ...
    path_tmp = os.path.abspath(sys.argv[1])
    print(f'Aligning images in {path_tmp}')
    ij = imagej.init(utils.get_Fiji_path(), mode='interactive')
