function[ImageMD,ImageMdelta,ImageMR,M11,total_diattenuation,linear_diattenuation,circular_diattenuation,total_depolarization,total_retardance,retardance_vector,linear_retardance,orientation_linear_retardance,orientation_linear_retardance_full,circular_retardance,physical_criterion]=polardecomposition0_paper_image_v4_with_SVD(MM)
...
%% =================  Extraction de la matrice du déphaseur  ================= %%
            ...
            [U_R,S_R,V_R]=svd(muel_0(2:4,2:4));
           

%             % general sign options to be selected
            % (+1/+1 must be selected if det(M) > 0 to ensure positive semi-definiteness!)
            s1 = 1;
            s2 = 1;
            % sign adjustment vector
            if sign(det(muel))<0
            s = [s1 s2 -(s1 * s2 )];
            else 
            s =[1 1 1];
