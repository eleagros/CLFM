...
ImageDep = Image_total_depolarization(mask_final(:));
ImageRet = Image_linear_retardance(mask_final(:));
ImageAzi = Image_orientation_linear_retardance_full(mask_final(:));
ImageDia = Image_linear_diattenuation(mask_final(:));
...
parametre_polarimetrique4 = 'Linear diattenuation';


limites_pas_histogrammeDep = [0 0.001 1.2];
limites_pas_histogrammeRet = [0 0.1 60];
limites_pas_histogrammeAzi = [0 1 180];
limites_pas_histogrammeDia = [-0.1 0.001 0.4];

couleur= [14.9/100, 25.9/100, 54.9/100];
position_text_meanDep = [0.2 1.25];
position_text_sigmaDep = [0.2 1.2];
position_text_maxkarDep = [0.2 1.15];
...
vecDia = vecDia((vecDia>limites_pas_histogrammeDia(1))&(vecDia<limites_pas_histogrammeDia(3)));
...
[countsDia,binsDia]=hist(vecDia,[limites_pas_histogrammeDia(1):limites_pas_histogrammeDia(2):limites_pas_histogrammeDia(3)],'FaceColor','blue');
...
subplot(2,2,4);
%histogram_polarimetric_markers(Image_total_depolarization(mask_final(:)),'Total depolarization',[0 0.001 1.1],[81.6/100, 65.9/100, 14.5/100],[0.5 1.25],[0.5 1.2],[0.5 1.15])
plot(binsDia,countsDia./max(countsDia),'Linewidth',1,'Color',couleur);
val_moyenne=mean(vecDia);
ecart_type=std(vecDia);
[maxkarDia maxkar2Dia]= max(countsDia./max(countsDia))
...
str3 = strcat('max=',num2str(round2(maxkar2Dia/1000,0.001)));
text(position_text_maxkarDep(1),position_text_maxkarDep(2),str3)

xlim([limites_pas_histogrammeDia(1) limites_pas_histogrammeDia(3)]);
ylim([0 1.5]);
grid on;
grid minor;
xlabel(parametre_polarimetrique4);
ylabel('Pixel number normalizaed');
title(parametre_polarimetrique4,'FontSize',15,'FontName','Calibri');
%  end