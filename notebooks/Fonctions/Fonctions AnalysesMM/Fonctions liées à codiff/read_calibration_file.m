function varargout=read_calibration_file(varargin)
%| Description                : fonction de lecture d'un fichier cod sizeX,sizeY sans header au début
%|
%| Appel                      : [M_mueller]=read_cod_file(filename,pathname, size, header)
%|
...
if nargin == 0
    ...
    if nargin == 2
        ...
        taille = d.bytes;
        if taille == 256*256*16*4
            sizeX=256;
            sizeY=256;
        elseif taille == 512*512*16*4
            sizeX=512;
            sizeY=512;
        elseif taille == 600*800*16*4   %+140*4 si header = 140
            sizeX=800;
            sizeY=600;
        end
        header = 0;
